#include "drv_sauter_heater.h"

//#define DEBUG_SAUTER
#ifdef DEBUG_SAUTER
#	define LOG_DEBUG_SAUTER(...)				log_debug(__VA_ARGS__)
#	define LOG_DEBUG_SAUTER(...)				log_error(__VA_ARGS__)
#else
...
ret_code_t drv_sauter_heater_send(sauter_heater *aCtx, BOOL afOn, float aTemperature, float aBoostTimeMinute)
{
	...
	buf[SAUTER_HEATER_OFFSET_CRC] = drv_sauter_heater_crc(buf, sizeof(buf) - 2); //-2 = Footer + CRC
}
